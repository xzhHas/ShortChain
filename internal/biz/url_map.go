...
type UrlMapRepo interface {
	...
	// 查询短链（从缓存）
	GetShortUrlFormCache(context.Context, string) (string, error) // v3

	CreateToDb(context.Context, *UrlMap) (int64, error)

	SaveToDb(context.Context, *UrlMap) error
	SaveToCache(context.Context, string, string) error // v3
	...
	FindShortUrlFormBloomFilterAndCache(context.Context, string) (int64, string, error) // v3
}
